  ...
  return (
    <div className="mx-auto mt-12 flex max-w-5xl flex-wrap gap-4 p-4 sm:grid sm:grid-cols-[auto_1fr]">
      ...
      {callToAction && callToAction?.length > 0 && (
        <div className="motion-translate-y-in-[2rem] motion-opacity-in-[0%] motion-delay-2000 col-start-1 flex flex-wrap items-start gap-2 sm:max-sm:row-start-3 md:items-end">
      ...
      {image && isMedia(image) && (
        <div className="motion-translate-x-in-[183%] motion-scale-in-[.5] motion-opacity-in-[0%] motion-translate-y-in-[12%] motion-delay-2000 motion-duration-1000 relative col-start-2 row-span-2 row-start-2 ml-auto max-xl:mr-20 max-md:h-40 max-md:w-40 md:h-52 md:w-52">
          <Image
            src={image.url ?? ""}
            alt={image.alt ?? ""}
